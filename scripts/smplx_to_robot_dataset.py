...
def check_memory(min_available_gb=4.0):  # pause only when available memory is too low
    mem = psutil.virtual_memory()
    used_memory_gb = (mem.total - mem.available) / (1024 ** 3)
    available_memory_gb = mem.available / (1024 ** 3)
    if min_available_gb > 0 and available_memory_gb < min_available_gb:
        print(
            f"[WARNING] Memory usage:{used_memory_gb:.2f} GB, available:{available_memory_gb:.2f} GB, "
            f"below minimum available threshold {min_available_gb} GB."
        )
        return True
    return False
...
def process_file(
    ...
    num_pause = 0
    while check_memory(min_available_gb=min_available_gb):
...
def main():
    ...
    parser.add_argument(
        ...
        default=4.0,
        type=float,
        help="Pause workers only when available system RAM is below this value. Set <=0 to disable pause.",
    )
    parser.add_argument(
        "--pause_seconds",
        default=120,
        type=int,
        help="Sleep duration in seconds when memory pause is triggered.",
    )
        ...
        for filename in natsorted(filenames):
            ...
            if filename.endswith((".pkl", ".npz")):
                ...
                if not os.path.exists(tgt_file_path) or args.override:
                    args_list.append((smplx_file_path, tgt_file_path, args.robot, args.config, SMPLX_FOLDER, tgt_folder))
    ...
    with mp.Pool(args.num_cpus) as pool:
        pool.starmap(
            process_file,
            [
                file_args + (total_files, verbose, args.min_available_gb, args.pause_seconds)
                for file_args in args_list
            ],
        )
