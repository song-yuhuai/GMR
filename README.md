...
python scripts/calibrate_ik_offsets.py   --template general_motion_retargeting/ik_configs/bvh_to_g1.json   --robot unitree_g1   --edit left_shoulder_roll_link   --set_pos 0 0 0   --set_euler 0 0 0   --output general_motion_retargeting/ik_configs/bvh_to_g1.json

python scripts/calibrate_ik_offsets.py   --template general_motion_retargeting/ik_configs/bvh_to_gp02_v2.json   --robot gp02_v2   --edit left_ankle_roll_link --edit right_ankle_roll_link   --edit left_knee_link --edit right_knee_link   --set_pos 0 0 0   --output general_motion_retargeting/ik_configs/bvh_to_gp02_v2_manual.json

python scripts/compare_ik_offsets.py --config general_motion_retargeting/ik_configs/bvh_lafan1_to_g1.json --robot unitree_g1 --motion_file MotionData/lafan1/fallAndGetUp2_subject2.bvh --after_ik --vis --show_frames

# For SMPL-X configs (e.g. smplx_to_*.json), you still need a motion file (.npz)
# to provide frame-wise joint targets for comparison.
# IK config rotation offsets support:
# - quaternion [w, x, y, z]
# - Euler xyz in degrees [rx, ry, rz]
# - dict form {"quat":[...]} / {"euler_deg":[...]} / {"euler_rad":[...]}
python scripts/compare_ik_offsets.py   --config general_motion_retargeting/ik_configs/smplx_to_agibot_x2.json   --robot agibot_x2   --motion_file ACCAD/walk_and_run/C11_-_run_turn_left_90_stageii.npz   --after_ik   --trajectory_eval   --frame 0 --num_frames 120 --frame_step 1   --use_velocity_limit   --joint_step_limit 0.12   --joint_blend 0.15 --vis --show_frames

python scripts/bvh_to_robot.py --bvh_file MotionData/lafan1/fallAndGetUp2_subject2.bvh --robot unitree_g1 --save_path GMR/RetargetData/lafan1/unitree_g1/fallandgetup2_subject2.pkl --rate_limit  

python scripts/bvh_to_robot.py   --bvh_file MotionData/lafan1/fallAndGetUp2_subject2.bvh --robot gp02_v2 --save_path GMR/RetargetData/lafan1/gp02_v2/fallandgetup2.pkl --rate_limit --align_ground  
```

### Automatic IK Offset Search (Recommended for hard arm alignment)

Use `scripts/search_ik_link_offsets.py` to search `pos_offset` + rotation increments for selected links
(e.g., elbow/wrist) and save a new config.

Quick coarse search:
```bash
python scripts/search_ik_link_offsets.py \
  --config general_motion_retargeting/ik_configs/smplx_to_agibot_x2.json \
  --robot agibot_x2 \
  --motion_file ACCAD/walk_and_run/C14_-_run_turn_right_90_stageii.npz \
  --links pelvis,torso_link,left_shoulder_yaw_link,left_elbow_link,left_wrist_yaw_link,right_shoulder_yaw_link,right_elbow_link,right_wrist_yaw_link \
  --max_frames 10 --frame_step 4 \
  --pos_range 0.0 --pos_step 0.05 \
  --rot_range_deg 90 --rot_step_deg 10 \
  --objective_ori_weight 0.002 \
  --progress_every 20 \
  --isolate_links \
  --reset_every 0 \
  --checkpoint_config /tmp/x2_search_checkpoint.json \
  --save_config general_motion_retargeting/ik_configs/smplx_to_agibot_x2_search_quick.json
```

```bash
python scripts/search_ik_link_offsets.py \
  --config general_motion_retargeting/ik_configs/bvh_lafan1_to_x2.json \
  --robot agibot_x2 \
  --motion_file MotionData/lafan1/fallAndGetUp2_subject2.bvh \
  --links left_elbow_link,left_wrist_roll_link,right_elbow_link,right_wrist_roll_link \
  --max_frames 10 \
  --frame_step 4 \
  --pos_range 0.01 \
  --pos_step 0.01 \
  ...
  --objective_ori_weight 0.002 \
  --progress_every 50 \
  --save_config general_motion_retargeting/ik_configs/bvh_lafan1_to_x2_search_quick.json
```

Validate searched config:
```bash
python scripts/compare_ik_offsets.py \
  --config general_motion_retargeting/ik_configs/bvh_lafan1_to_x2_search_quick.json \
  --robot agibot_x2 \
  --motion_file MotionData/lafan1/fallAndGetUp2_subject2.bvh \
  --after_ik --vis --show_frames
```

Then run a finer search around the best config by reducing `pos_range/rot_range_deg` and `step` values.
...
python scripts/bvh_to_robot.py --bvh_file GMR/MotionData/lafan1/{your_bvh_file}.bvh --robot pi_football --save_path GMR/RetargetData/lafan1/pi_football/{your_bvh_file}.pkl --rate_limit
```
