...
def load_smplx_file(smplx_file, smplx_body_model_path):
    ...
    try:
        ...
        if "hands_componentsl" in str(e):
            gender = str(smplx_data["gender"]).upper()
            model_dir = os.path.join(str(smplx_body_model_path), "smplx")
            expected_npz = os.path.join(model_dir, f"SMPLX_{gender}.npz")
            expected_pkl = os.path.join(model_dir, f"SMPLX_{gender}.pkl")
            raise RuntimeError(
                "SMPL-X model file is incomplete. Missing hand PCA fields "
                "(e.g., hands_componentsl/hands_componentsr). "
                f"Please place full official SMPL-X model files at {model_dir} "
                f"(prefer {expected_npz}, or complete {expected_pkl})."
            ) from e
        raise
    except TypeError:
        body_model = smplx.create(
...
def load_gvhmr_pred_file(gvhmr_pred_file, smplx_body_model_path):
    ...
    try:
        ...
        if "hands_componentsl" in str(e):
            model_dir = os.path.join(str(smplx_body_model_path), "smplx")
            expected_npz = os.path.join(model_dir, "SMPLX_NEUTRAL.npz")
            expected_pkl = os.path.join(model_dir, "SMPLX_NEUTRAL.pkl")
            raise RuntimeError(
                "SMPL-X model file is incomplete. Missing hand PCA fields "
                "(e.g., hands_componentsl/hands_componentsr). "
                f"Please place full official SMPL-X model files at {model_dir} "
